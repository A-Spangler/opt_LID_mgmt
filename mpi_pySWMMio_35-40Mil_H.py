...
def pySWMMio(*vars):
    ...
    #_______________________Organize variables and folder names______________________
    for i in range(156):
    ...
    subbasin_attributes = pd.read_csv(r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/26SubBasinAttributes11July2022.csv")
    subbasin_attributes = subbasin_attributes.set_index('Subbasin')
    swmm_folder = (r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/26SBMeadowCreekSWMM_ShortTerm")
    swmmio_altered_inp_file_name = (r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/SWMMIO_Altered_INPUTS/35-40 Mil")
    pyswmm_results_txt =  (r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/SWMM_Results/35-40 Mil Results")
    upper_cost_limit = 40000000
    lower_cost_limit = 35000000

    #_________________Create duplicate files for parallel simulation_________

    rando_n = str(randint(0,90000000))
...
# # Calculating Grass Swale Costs
    ...
    for subbasin in subbasin_attributes.index:
        subbasin_attributes.at[subbasin,'Imp_Removal_Costs'] = 5*(((subbasin_attributes.at[subbasin,'Perc_Imperv']/100)*subbasin_attributes.at[subbasin,'Areas'])- subbasin_attributes.at[subbasin,'Updated_Imp_Areas'])
...
#SWMMIO input file modifications
    ...
    with Simulation(swmmio_altered_inp_file_name+altered_SWMMIO_inp, reportfile = pyswmm_results_txt+output_text_file) as sim:
...
# _________________Get average peak flow across 10 stream reaches_____________________________________________________________
    ...
    rpt_43811 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =31186, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43771 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =31035, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43631 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =30431, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43581 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =30129, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43481 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows = 29525, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python') 
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43391 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =28921, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43301 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =28317, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43241 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =28166, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_43201 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =27562, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
    ...
    reportColumns = [str(i) for i in np.arange(0,8)]
    rpt_42831 = pd.read_csv(pyswmm_results_txt+output_text_file, skiprows =26203, nrows = 144, sep="   |    ", names = reportColumns, header = None, engine = 'python')    
 ...
 # _________  # Get Total Runoff Score
    ...
    os.remove(pyswmm_results_txt+output_text_file)
...
borg.setEpsilons(0.1,10)


result = borg.solveMPI(maxEvaluations=150000,runtime='mpi_pySWMMio_35-40Mil_H_epsilon.runtime', frequency = 1500)


if result:

    # Create/write objective values and decision variable values to files in folder "sets", 1 file per seed.
    f = open('mpi_pySWMMio_35-40Mil_H_epsilon.set', 'w')
    ...
    # raw_optimal_decision_lists = []
    # raw_optimal_objective_value_lists = []

    # for solution in result:
        
    #     raw_optimal_decision_lists.append(solution.getVariables())
    #     raw_optimal_objective_value_lists.append(solution.getObjectives())



    # subbasin_attributes_list = []
    # for sol in raw_optimal_decision_lists:

    #     raw_optimal_decision_list = sol
    #     subbasin_attributes = pd.read_csv(r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/26SubBasinAttributes11July2022.csv")
    #     subbasin_attributes = subbasin_attributes.set_index('Subbasin')
    #     subbasin_attributes['BorgPermeablePavementDecision'] = ''
    #     subbasin_attributes['BorgGrassSwaleDecision'] = ''
    #     subbasin_attributes['BorgGreenRoofDecision'] = ''
    #     subbasin_attributes['BorgBioretentionDecision'] = ''
    #     subbasin_attributes['BorgRainGardenDecision'] = ''
    #     subbasin_attributes['BorgImpRemovalDecision']= ''
    #     subbasin_list = subbasin_attributes.index.tolist()
    #     ninetyone_split = np.array_split(sol,26)

    #     optimal_decision_lists = []


    #     for array in ninetyone_split:
    #         optimal_decision_lists.append(array.tolist())

    #     counter = 0
    #     for item in optimal_decision_lists:
    #         l = item
    #         subbasin_attributes.at[subbasin_list[counter],'BorgPermeablePavementDecision'] = l[0]
    #         subbasin_attributes.at[subbasin_list[counter],'BorgGrassSwaleDecision'] = l[1]
    #         subbasin_attributes.at[subbasin_list[counter],'BorgGreenRoofDecision'] = l[2]
    #         subbasin_attributes.at[subbasin_list[counter],'BorgBioretentionDecision'] = l[3]
    #         subbasin_attributes.at[subbasin_list[counter],'BorgRainGardenDecision'] = l[4]
    #         subbasin_attributes.at[subbasin_list[counter],'BorgImpRemovalDecision'] = l[5]

    #         counter += 1

    #     subbasin_attributes['PermeablePavement Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'PermeablePavement Assigned Area'] = subbasin_attributes.at[index,'BorgPermeablePavementDecision']*subbasin_attributes.at[index,'Feasible_PermeablePavement_Space']

    #     subbasin_attributes['GrassSwale Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'GrassSwale Assigned Area'] = subbasin_attributes.at[index,'BorgGrassSwaleDecision']*subbasin_attributes.at[index,'Feasible_GrassSwale_Space']

    #     subbasin_attributes['GreenRoof Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'GreenRoof Assigned Area'] = subbasin_attributes.at[index,'BorgGreenRoofDecision']*subbasin_attributes.at[index,'Feasible_GreenRoof_Space']

    #     subbasin_attributes['Bioretention Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'Bioretention Assigned Area'] = subbasin_attributes.at[index,'BorgBioretentionDecision']*subbasin_attributes.at[index,'Feasible_Bioretention_Space']

    #     subbasin_attributes['RainGarden Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'RainGarden Assigned Area'] = subbasin_attributes.at[index,'BorgRainGardenDecision']*subbasin_attributes.at[index,'Feasible_RainGarden_Space']

    #     subbasin_attributes['Imp Removal Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'Imp Removal Assigned Area'] = (subbasin_attributes.at[index,'Perc_Imperv']*subbasin_attributes.at[index,'Areas']/100)-((subbasin_attributes.at[index,'Perc_Imperv']-subbasin_attributes.at[index,'Removable_Imp']*subbasin_attributes.at[index,'BorgImpRemovalDecision'])*subbasin_attributes.at[index,'Areas']/100)

    #     subbasin_attributes['Total LID Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'Total LID Assigned Area'] = subbasin_attributes.at[index,'PermeablePavement Assigned Area'] + subbasin_attributes.at[index,'GrassSwale Assigned Area'] + subbasin_attributes.at[index,'GreenRoof Assigned Area']+subbasin_attributes.at[index,'Bioretention Assigned Area']+subbasin_attributes.at[index,'RainGarden Assigned Area']+subbasin_attributes.at[index,'Imp Removal Assigned Area']

    #     subbasin_attributes['Total Green LID Assigned Area'] = ''
    #     for index, rows in subbasin_attributes.iterrows():
    #         subbasin_attributes.at[index,'Total Green LID Assigned Area'] = subbasin_attributes.at[index,'GrassSwale Assigned Area'] + subbasin_attributes.at[index,'GreenRoof Assigned Area']+subbasin_attributes.at[index,'Bioretention Assigned Area']+subbasin_attributes.at[index,'RainGarden Assigned Area']+subbasin_attributes.at[index,'Imp Removal Assigned Area']

    #     # subbasin_attributes['Percent of LID Space Assigned'] = ''
    #     # for index, rows in subbasin_attributes.iterrows():
    #     #     if subbasin_attributes.at[index,'Total_LID_Feasible_Area'] == 0:
    #     #         pass
    #     #     else:
    #     #          subbasin_attributes.at[index,'Percent of LID Space Assigned'] = (subbasin_attributes.at[index,'Total LID Assigned Area']/subbasin_attributes.at[index,'Total_LID_Feasible_Area'])*100

    #     subbasin_attributes_list.append(subbasin_attributes) 

    # sol_number_list = list(range(len(subbasin_attributes_list)))
    # for sol in sol_number_list:
    #     subbasin_attributes_list[sol].to_csv(r"/sfs/lustre/bahamut/scratch/rsh6pb/C/Borg/Borg-1.8/Borg Results/35-40 Mil/35-40MilSol_H_#{}.csv".format(sol))

    # print(raw_optimal_objective_value_lists)
